...
module PE_tb();
    ...
    initial begin
        ...
        //====================================================================
        // Test 6: Test start signal rising edge trigger
        //====================================================================
        $display("\n[Test 6] Test start rising edge trigger");
        ...
        // Wait for several cycles first
        repeat(3) @(posedge clk);
        
        // Pull start high, should restart calculation
        start = 1;
        $display("T=%0t: start rising edge, reinitialize", $time);
        
        @(posedge clk);
        $display("T=%0t: 1st MAC, psum_out=%0d", $time, psum_out);
        
        @(posedge clk);
        $display("T=%0t: 2nd MAC, psum_out=%0d", $time, psum_out);
        
        @(posedge clk);
        $display("T=%0t: 3rd MAC, psum_out=%0d", $time, psum_out);
        
        start = 0;
        @(posedge clk);
        display_result(6, "start rising edge trigger test");
